...
package net.mcreator.easycrafting.keybind;
...
import net.mcreator.easycrafting.procedures.OpenGUIOnKeyPressedProcedure;
...
public class OpenGUIKeyBinding extends OpitemsModElements.ModElement {
	@OnlyIn(Dist.CLIENT)
	private KeyBinding keys;
	public OpenGUIKeyBinding(OpitemsModElements instance) {
		super(instance, 2);
	...
	public void initElements() {
		keys = new KeyBinding("key.mcreator.open_gui", GLFW.GLFW_KEY_F8, "key.categories.misc");
		...
		if (Minecraft.getInstance().currentScreen == null) {
			if (event.getKey() == keys.getKey().getKeyCode()) {
				if (event.getAction() == GLFW.GLFW_PRESS) {
					OpitemsMod.PACKET_HANDLER.sendToServer(new KeyBindingPressedMessage(0, 0));
		...
		if (type == 0) {
			{
				...
				OpenGUIOnKeyPressedProcedure.executeProcedure($_dependencies);
			}
		}
	}
}
